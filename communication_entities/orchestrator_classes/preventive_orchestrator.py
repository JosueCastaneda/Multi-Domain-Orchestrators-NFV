...
class PreventiveOrchestrator:

    def __init__(self, my_definition: OrchestratorDefinition):
        self.log = None
        ...
        random.seed(self.my_definition.random_seed)

    def add_vnf_forwarding_graph(self, vnf_forwarding_graphs):
        string_1 = ROOT_DIR + '/' + 'experiments/experiment_'
        directory_path = string_1 + str(self.my_definition.experiment_index) + '/' + 'experiment_' + str(self.my_definition.experiment_index)
        with open(directory_path + '.json') as json_file:
            raw_data = json.load(json_file)
        for orchestrator in raw_data['orchestrators']:
            self.my_definition.orchestrators_ids.append(orchestrator['id'])
        ...
        for vnf_fg_entry in vnf_forwarding_graphs:
            ...
            new_vnf_fg.instantiate_classification_rules(vnf_fg_entry, self.my_definition.orchestrators_ids)
            new_vnf_fg.instantiate_rendered_service_path(vnf_fg_entry, self.my_definition.orchestrators_ids)
    ...
    def load_server_information(self):
        all_route = ROOT_DIR + '/' + self.my_definition.directory_path + self.my_definition.experiment_name + '.json'
        with open(all_route) as json_file:
            raw_data = json.load(json_file)

        orchestrator_number = self.my_definition.name.find('_')
        orchestrator_index = int(self.my_definition.name[orchestrator_number + 1:])
    ...
    def find_orchestrator_by_id(self, id_orchestrator):
        ...
        my_orchestrator['ip'] = self.my_definition.ip
        my_orchestrator['port'] = self.my_definition.port
        my_orchestrator['name'] = self.my_definition.name
    ...
    async def get_messages_sent(self):
        return self.my_definition.messages_sent
    ...
    def entry_as_dictionary(self):
        ...
        orchestrator_format['ip'] = self.my_definition.ip
        orchestrator_format['port'] = self.my_definition.port
        orchestrator_format['id'] = self.id
        orchestrator_format['name'] = self.my_definition.name
        orchestrator_format['experiment_name'] = self.my_definition.experiment_name
        orchestrator_format['experiment_index'] = self.my_definition.experiment_index
        orchestrator_format['is_causal'] = self.my_definition.causal_delivery
        orchestrator_format['algorithm_type'] = self.my_definition.algorithm_type
        orchestrator_format['vector_clock'] = self.vector_clock.as_string()
        orchestrator_format['inconsistencies'] = self.inconsistencies
        orchestrator_format['random_seed'] = self.my_definition.random_seed
        orchestrator_format['messages_sent'] = self.my_definition.messages_sent
        ...
        orchestrator_format['total_time_for_experimentation'] = self.my_definition.total_time_for_experimentation
    ...
    async def wait_before_notification(self, data):
        wait_period = random.randint(0, self.my_definition.waiting_time)
        await asyncio.sleep(wait_period)
        await self.notification_of_lcm(data)

    def set_up_my_logger(self):
        file_name = 'logs/file_' + self.my_definition.name + '_' + self.my_definition.experiment_index + '.log'
    ...
    async def notification_of_lcm(self, data):
        ...
        if self.my_definition.algorithm_type == 'causal':
            causality = True
        ...
        if my_clock_is_greater and self.my_definition.algorithm_type == 'causal':
            pass
        else:
    ...
    async def apply_notification_of_lifecycle_management_operation(self, vector_difference, vector_clock, sender, data=None, my_clock_is_greater=False):
        ...
        elif self.my_definition.algorithm_type == 'standard':
        ...
        elif self.my_definition.algorithm_type == 'last_writer_wins':
            await self.notification_of_lcm_last_writer_wins(vector_difference, vector_clock, sender)
        elif self.my_definition.algorithm_type == 'multi_value':
    ...
    async def apply_notification_of_lcm_operation(self, vector_clock, orchestrator_sender_id, data=None):
        ...
        if not my_clock.is_equal(vector_clock):
            ...
            self.my_definition.total_time_for_experimentation += time_difference
    ...
    async def notify_all_orchestrators_of_change(self, result):
        ...
        data['type'] = self.my_definition.algorithm_type
        data['orchestrator_counter'] = self.my_definition.orchestrator_counter
        data['orchestrator_index'] = self.my_definition.orchestrator_index
    ...
    def increment_sent_messages(self):
        self.my_definition.messages_sent += 1
    ...
    async def repeat_lcm_operations(self):
        ...
        while self.pending_lcm_operations and message_repetitions_are_lesser and my_repetitions_are_lesser:
            ...
            elif self.my_definition.algorithm_type == 'normal':
    ...
    async def repeat_lcm_operations_causal(self, difference_in_clocks, pending_operation):
        ...
        if difference_in_clocks <= 1 and pending_operation.is_not_done:
            ...
            if change_took_place:
                end_time = time.time()
                self.my_definition.total_time_for_experimentation += (end_time - pending_operation.vnffg_data['buffer_time'])
        else:
    ...
    async def grant_lcm_operation_of_service(self, vnf_component_to_scale_id, operation, original_service,
                                             sender_vector_clock=None):
        if self.my_definition.causal_delivery:
    ...
    async def repeat_operation_store_clock(self):
        ...
        while self.pending_lcm_operations and message_repetitions_are_lesser and my_repetitions_are_lesser:
            ...
            if operation.is_not_done and self.vector_clock.compare_stored_clock(operation.sender_vector_clock,
                                                                                self.my_definition.causal_delivery):
    ...
    def load_vnf_components(self):
        ...
        if self.my_definition.ip == '127.0.0.1':
            my_ip = self.my_definition.ip
            for vnf in self.vnfs:
                vnf['server'] = self.my_definition.ip
        else:
    ...
    def instantiate_services(self, services, ip):
        for service in services:
            new_service = GenericService(service['id'],
                                         ...
                                         ip,
                                         self.my_definition.port)
            self.services.append(new_service)

    def print_state_vnf(self):
        self.log.info(''.join(["VNF name: ", self.my_definition.name]))
    ...
    async def update_unique_vnf_forwarding_graph_rendered_service_path(self, connection_point: ConnectionPointReference):
        for vnf_forwarding_graph in self.vnf_forwarding_graphs:
            if len(connection_point.list_of_orchestrator_id) == 0:
                connection_point.list_of_orchestrator_id = self.my_definition.orchestrators_ids
                connection_point.add_clocks()

            list_caller = dict()
            list_caller['caller'] = self.id
            list_caller['first_call'] = True
            update_result = await vnf_forwarding_graph.update_unique_rendered_service_path(connection_point, self.log, self.my_definition.orchestrator_index, list_caller)
    ...
    async def update_unique_vnf_forwarding_graph_classifier_rule(self, attribute: MatchingAttribute):
        if len(attribute.list_of_orchestrator_id) == 0:
            attribute.list_of_orchestrator_id = self.my_definition.orchestrators_ids
    ...
    async def notify_all_replicas_and_orchestrators_of_positive_change(self, result):
        ...
        self.my_definition.total_time_for_experimentation += result['running_time']
        return return_success()

    def increase_my_counter(self):
        self.my_definition.orchestrator_counter += 1
        ...
        for replica in data['replicas']:
            for orchestrator in self.list_orchestrator:
                if orchestrator['id'] == replica:
                    ...
                    vnf_forwarding_graph['type'] = self.my_definition.algorithm_type
                    vnf_forwarding_graph['orchestrator_index'] = self.my_definition.orchestrator_index
                    ...
                    if self.my_definition.probability_repeated_message >= random_number:
                        concurrent_updates = []
                        self.increment_sent_messages()
                        concurrent_updates.append(send_message(new_message))
                        while self.my_definition.probability_repeated_message >= random_number:
        ...
        for vnf_fg_entry in self.vnf_forwarding_graphs:
            ...
            if is_same_identifier or is_same_identifier_alternate:
                ...
                if my_current_max_index == -1:
                    my_current_max_index = self.my_definition.orchestrator_index
    ...
    async def apply_vnf_fg_update_notification(self, vnf_forwarding_graph):
        for vnf_fg_entry in self.vnf_forwarding_graphs:
            ...
            if is_same_identifier or is_same_identifier_alternate:
                ...
                self.my_definition.total_time_for_experimentation += (end - start)
                break
    ...
    def create_vnf_fg_update_entry_classifier(self, new_vnf_forwarding_graph):
        new_matching_attributed = MatchingAttribute(
            ...
            list_of_orchestrator_id=self.my_definition.orchestrators_ids)
    ...
    async def update_vnf_forwarding_graph_unique_rsp(self,
        ...
        result = await old_vnf_fg.update_unique_rendered_service_path(new_cp, self.log, self.my_definition.orchestrator_index)
        if self.my_definition.algorithm_type == 'last_writer_wins':
            self.log.info(
                'After update: {' + str(self.my_definition.orchestrator_index) + ',' + str(result['new_counter']) + ',' + str(
    ...
    async def apply_concurrent_updates(self):
        ...
        for update in vnf_forwarding_graph_updates:
            if update['host'] == self.my_definition.ip and update['port'] == str(self.my_definition.port) and update['type'] != 'update_all_vnffg':
    ...
    def read_vnf_forwarding_graph_updates(self):
        updates = []
        string_1 = ROOT_DIR + '/experiments/experiment_'
        directory_path = string_1 + str(self.my_definition.experiment_index) + '/' + 'updates_vnf_forwarding_graphs.sh'
