...
async def init_app(port, is_master=False) -> web.Application:
    print('Server running in 0.0.0.0:' + str(port))
    app = web.Application()
    handler = ExperimentHandler(is_master)
...
def main(argv) -> None:
    ...
    if debug:
        server_port = 7001
    try:
        valid_arguments_as_string = "p:l:"
        list_valid_arguments = ["port=", "local="]
